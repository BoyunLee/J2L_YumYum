  ...
  <div v-else class="admin-shell">
    ...
    <main class="admin-main">
      ...
      <section v-else-if="activeTab === 'usage'" class="admin-content">
        ...
        <div class="admin-stats compact"><article class="blue"><p>전체 호출</p><strong>{{ usage?.totalCalls.toLocaleString() ?? 0 }}</strong><small>최근 24시간</small></article><article class="green"><p>성공률</p><strong>{{ usageSuccessRate }}%</strong><small>{{ usage?.successfulCalls ?? 0 }}건 성공</small></article><article class="orange"><p>OCR</p><strong>{{ usage?.points.reduce((sum, point) => sum + point.ocr, 0) ?? 0 }}</strong><small>문서 인식</small></article><article class="purple"><p>AI 추천</p><strong>{{ usage?.points.reduce((sum, point) => sum + point.recipeRecommendation, 0) ?? 0 }}</strong><small>레시피 생성</small></article></div>
...
@media(max-width:900px){.admin-sidebar{position:sticky;top:0;width:100%;height:auto;display:block;padding:.7rem;overflow:auto}.admin-brand,.admin-account{display:none}.admin-sidebar nav{display:flex}.admin-sidebar nav button{white-space:nowrap}.admin-main{margin-left:0}.admin-topbar{display:none}.admin-stats{grid-template-columns:repeat(2,1fr)}.admin-grid.two,.notice-grid{grid-template-columns:1fr}.admin-content{padding:1.2rem}.admin-shell{display:block}}
@media(max-width:520px){.admin-stats{grid-template-columns:1fr}.admin-page-head{align-items:flex-start;gap:1rem;flex-direction:column}.admin-sidebar nav button{padding:.65rem}.admin-sidebar nav button i{display:none}.line-chart{height:13rem}.chart-axis-labels small{font-size:.48rem}}
</style>
